...
This project contains source code and supporting files for a basic Salesforce.com REST API implementation built for AWS API Gateway, you can deploy with the SAM CLI. It includes [Simple Salesforce client to REST API connections ](https://github.com/simple-salesforce/simple-salesforce), [Lambda Powertools for operational best practices](https://github.com/awslabs/aws-lambda-powertools-python), and the following files and folders.

- **`leads_salesforce`** - Code for the application's Lambda function.
...
```bash
leads-salesforce$ make build
leads-salesforce$ make deploy.guided
```
...
The SAM CLI installs dependencies defined in `leads_salesforce/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.

Test a single function by invoking it directly with a test event:

```bash
leads-salesforce$ make invoke
```
...
```bash
leads-salesforce$ make run
leads-salesforce$ curl http://localhost:3000/lead
```
...
```bash
leads-salesforce$ sam logs -n LeadsSalesforceFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
leads-salesforce$ make dev
leads-salesforce$ make test
```
...
```bash
leads-salesforce$ aws cloudformation delete-stack --stack-name leads-salesforce
```

# Appendix

## Simple Salesforce
[Simple Salesforce](https://awslabs.github.io/aws-lambda-powertools-python/core/tracer/) is a basic Salesforce.com REST API client built for Python 3.5, 3.6, 3.7 and 3.8. The goal is to provide a very low-level interface to the REST Resource and APEX API, returning a dictionary of the API JSON response.
...
* **`make invoke`**: `sam local invoke LeadsSalesforceFunction --event events/leads_salesforce_event.json`
...
```bash
leads-salesforce$ pipenv lock -r > leads_salesforce/requirements.txt
```
